...
class VectorStore:
    ...
    def save_index(self, index_name='vector_index'):
        ...
        try:
            ...
            logging.error(f"Error saving index to Supabase: {e}")
            return False
    ...
    def index_exists(self, index_name='vector_index'):
        ...
        try:
            ...
            # Try to list files in indexes directory
            try:
                files = supa.list_files(prefix="indexes/")
                index_files = [f.get('name', '') for f in files]
                return f"indexes/{index_name}.faiss" in index_files and f"indexes/{index_name}.meta" in index_files
            except Exception:
                return False
        except Exception:
            return False
